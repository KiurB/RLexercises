...
/* Results are a bit different from the book, but the value function makes
perfect sense. Probably it's due to the way I have set up the problem with the
skellam distribution and cumulative distributions to get sum of probabilities
in the transition matrix equal 1 */
...
{   
	...
	int policy_table[maxcars][maxcars];
	for (int i=0;i<=maxcars*maxcars;++i){
	...
	//visualize the value
	long double value_table[maxcars][maxcars];
	for (int i=0;i<=maxcars*maxcars;++i){
		value_table[states[i][0]][states[i][1]] = values[i];
	}
	for(int i=0;i<maxcars;i++){
	  	for(int j=0;j<maxcars;j++)
	  	{
	  		std::cout<<" "<< std::setw(10) << value_table[i][j];
		}
		std::cout<<std::endl; 
	}
